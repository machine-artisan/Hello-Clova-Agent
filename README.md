...
원인: PyPI Python 3.12용 vLLM 휠이 CUDA 13으로 컴파일됩니다.
  ...
  2단계 — version 'libcudart.so.13' not found
      ...
      → 심볼릭 링크로는 해결 불가. 파일명이 아닌 이진 레벨 의존성입니다.

현재 상태: 미해결.
      실용적 해결책은 실제 CUDA 13 런타임 또는 patchelf 바이너리 패치이나
      Colab 환경에서 안정적으로 적용하는 방법이 확인되지 않았습니다.
      국산 LLM 사용 요건이 없다면 Ollama + Qwen 계열을 사용하세요.
```
